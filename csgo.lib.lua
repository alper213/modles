...
do
    function library.init(title, version, id, position, size)
        ...
        -- 1) getgenv().watermark == false -> watermark oluşturma
        -- 2) getgenv().watermark bir GuiObject (Instance) -> o GUI MasterContainer içine parent edilir
        -- 3) getgenv().watermark bir table -> içindeki alanlara göre oluşturulur
        --    alanlar: text, size, anchor, position, rotation, zindex, textTransparency, textColor, font, hoverFade (true/false), textStrokeColor, textStrokeTransparency
        local wmCfg = getgenv().watermark
        local Watermark
        if wmCfg == false then
            Watermark = nil
        elseif typeof(wmCfg) == "Instance" and wmCfg:IsA("GuiObject") then
            -- Eğer dışarıda oluşturulmuş GUI verilmişse, parent et
            wmCfg.Parent = MasterContainer
            Watermark = wmCfg
        else
            -- varsayılanlar
            local default = {
            ...
            wmCfg = type(wmCfg) == "table" and wmCfg or {}
            local text = wmCfg.text or default.text
            local sizeW = wmCfg.size or default.size
            local anchor = wmCfg.anchor or default.anchor
            local positionW = wmCfg.position or default.position
            local rotation = (wmCfg.rotation ~= nil) and wmCfg.rotation or default.rotation
            local zindex = wmCfg.zindex or default.zindex
            local textTransparency = (wmCfg.textTransparency ~= nil) and wmCfg.textTransparency or default.textTransparency
            local textColor = wmCfg.textColor or default.textColor
            local textSize = wmCfg.textSize or default.textSize
            local font = wmCfg.font or default.font
            local textStrokeColor = wmCfg.textStrokeColor or default.textStrokeColor
            local textStrokeTransparency = (wmCfg.textStrokeTransparency ~= nil) and wmCfg.textStrokeTransparency or default.textStrokeTransparency
            local hoverFade = (wmCfg.hoverFade ~= nil) and wmCfg.hoverFade or default.hoverFade

            Watermark = util.new("TextLabel", {
                Parent = MasterContainer,
                Size = sizeW,
                AnchorPoint = anchor,
                Position = positionW,
            ...
            Watermark.TextStrokeTransparency = textStrokeTransparency
            Watermark.TextStrokeColor3 = textStrokeColor

            if hoverFade then
                table.insert(connections, Watermark.MouseEnter:Connect(function()
                    util.tween(Watermark, { TextTransparency = 0.15 }, 0.12)
                end))
                table.insert(connections, Watermark.MouseLeave:Connect(function()
                    util.tween(Watermark, { TextTransparency = textTransparency }, 0.12)
                end))
            end
        end

        -- expose watermark to external scripts via getgenv()[id].Watermark
        getgenv()[id].Watermark = Watermark
